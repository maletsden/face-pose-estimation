...
void FaceStorage::UpdateObjectInfo(const Face& face, std::chrono::high_resolution_clock::time_point currentTimestamp)
{
    ...
    auto& kalmanFilterUpperJaw = faceObjectInfo.KalmanFilterUpperJaw;
    auto& kalmanFilterLowerJaw = faceObjectInfo.KalmanFilterLowerJaw;
...
void FaceStorage::SmoothObject(Face& face)
{
    ...
    {
        pose.RotationVector =
            (cv::Mat_<DataType>(1, 3) << measurment.at<DataType>(0, 0), measurment.at<DataType>(1, 0), measurment.at<DataType>(2, 0));
        pose.TranslationVector =
            (cv::Mat_<DataType>(1, 3) << measurment.at<DataType>(3, 0), measurment.at<DataType>(4, 0), measurment.at<DataType>(5, 0));
